function [theta,ypred,P,phi,z] = rarx_r(D,si,f,lambda,M,theta0,P0,phi0,z0) 
%
...
for n=(k+1):(k+N)			% Estimation and simulation. 
   ...
   phiS = [-D.y(n-M-1) ; phi(1:(na-1)) ; ... 
           D.u(n-M-1) ; phi((na+1):(k-1))] ; 
   z = [f(n-1) ; z(1:(k-1))] ; 
   z_s = [f(n-M-1);z(1:(k-M-1))];
%   phi = [-D.y((n-1):-1:(n-na)) ; D.u((n-1):-1:(n-nb))] ; 
%   z = f((n-1):-1:(n-k)) ; 
   zeta = P*z ; 
   zetaS = P*z_s;
%    nk = csi/(lambda+phi'*csi) ; 		% Gain vector. 
   csi = P*zeta;
   P = P - (csi*phi'*P)/(1+phi'*csi);
   csi = P*zetaS;
   P=P+(csi*phiS'*P)/(1-phiS'*csi);
   P = (P-nk*phi'*P)/lambda ; 		% Next matrix. 
   
   ypredD(n - k) = phi'*si ; 		% Predicted output. 
   ypredS(n-k) = phiS'*si;
   epsilonD = D.y(n)-ypredD(n - k);
   epsilonS = D.y(n)-ypredS(n - k);
  
   si = si + P*(zeta*epsilonD-zetaS*epsilonS) ; 	% Upgrade parameters. 
